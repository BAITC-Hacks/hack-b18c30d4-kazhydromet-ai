...
Актуальное дополнение и доказательства Windows/Linux: [FINAL-CHECK.md](FINAL-CHECK.md).
Кейс Freedom «Граф денег». Правила ролей, THRESHOLDS и приоритет не менялись. Все 6 материалов организаторов совпали с локальными. Последующие дизайн, календарь, охват проверки и коммиты: `docs/DESIGN-codex.md`; запуск для жюри: `docs/JURY-CHECK.md`.
...
Команде: репетиция на проекторе и «Сдать решение» на платформе (инструкция, п. 10; название/описание в PITCH). Роли не откалиброваны по ground truth, маршруты не доказывают происхождение денег.
