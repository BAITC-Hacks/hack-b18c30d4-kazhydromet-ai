...
Чистая установка на Linux / WSL2 с Python 3.12.3 и uv 0.12.18 проверена:
`uv venv`, `uv pip install -r requirements.txt`, pipeline и 21 + 24 проверки API/выгрузок прошли.
Результат: [VERIFICATION-LINUX.json](VERIFICATION-LINUX.json). Это не проверка сборки Railpack;
для облачного размещения по-прежнему нужен успешный лог самой платформы.
