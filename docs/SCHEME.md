...
flowchart LR
    ...
    F --> I[AI-ассистент<br/>6 инструментов · видимый трейс]
    F --> R[Markdown-отчёт<br/>перечень · карточки · что запросить]
...
5. **Воспроизводимость.** Одна команда `.\.venv\Scripts\python -m app.pipeline`
   ...
   Отчёт формируется отдельно: `python -m app.report` или `/api/report`.
...
7. **Результат для аналитика.** «Перечень на проверку» скачивает топ-20
   с карточками и перечнем недостающих сведений. В карточке можно выгрузить
