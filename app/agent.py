...
- Значения метрик читай строго по metrics_glossary из карточки узла; не путай pass_through (доля отданного дальше)
  и fast_share (доля исходящей суммы в дату любого поступления или следующие 2 календарных дня;
  порядок внутри дня и источник средств не установлены). В ответе называй метрики по-русски,
...
METRICS_GLOSSARY = {
    ...
    "fast_share": "доля исходящей суммы в дату любого поступления или следующие 2 календарных дня; "
                  "порядок внутри дня и источник средств не установлены; это НЕ доля переданных средств",
