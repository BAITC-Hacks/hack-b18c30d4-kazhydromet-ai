...
2. **Неполный вход у seed.** Поиск начинается от 81 известных клиентов;
   ...
   У остальных узлов входящие также могут быть неполными. Если видимая
   исходящая сумма больше входящей (`B>A`), это не доказывает «приумножение»:
   карточка предлагает запросить входящие вне выборки.
