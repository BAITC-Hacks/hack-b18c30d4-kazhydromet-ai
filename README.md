...
В командном демо используется OpenAI (`LLM_PROVIDER=openai`). Опциональный адаптер NVIDIA не используется.
...
Текущее имя возвращает `/api/health`; проверенный живой ответ получен от
`gpt-4.1-mini`. Модель используется только в чате, роли рассчитываются локально.
...
| `O>0, 0,5≤p≤2` | `transit` | `min(1; 0,55+0,4F−0,2abs(1−p))` | «исходящие / видимые входящие — 81% (3 получ.); для 85% исходящей суммы есть поступление в тот же или один из двух предыдущих дней» |
