...
- Один локальный запуск `app.pipeline` читает три Parquet-файла, строит
  ...
  топ-50, 5,0 секунды внутри расчёта**. На другом ноутбуке время нужно
  измерить снова.
...
Запасной экран доступен по `/graph.html`; ссылка `/graph.html#<gid>`
открывает конкретный узел. Команды запуска ниже привязывают сервер
...
- **Схема сети.** Стрелки показывают направление перевода. По умолчанию
  ...
  выбор кластера — узлы этой группы. Переключатель «Вся сеть» показывает
  все 2 248 узлов и может нагружать браузер.
...
- **Рабочие блоки.** Топ-лист из 50 узлов фильтруется по роли;
  карточка показывает объяснение роли, входящие и исходящие связи, даты
  и пробелы в данных. Список кластеров открывает группу на схеме.
  Чат предлагает готовые вопросы, показывает ответ и следы вызванных
  инструментов; без ключа работает локальный демо-режим. На запасном
...
Адрес: <http://127.0.0.1:8000>. Остановить сервер: `Ctrl+C`.
...
из `web/vendor/`. Codex использовался для подготовки
