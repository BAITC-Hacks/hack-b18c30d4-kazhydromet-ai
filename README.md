...
- Экран `web/graph.html` открывается с главного адреса: граф с направленными
  рёбрами, поиск по `gid`, список приоритетов, карточка, кластеры,
  устойчивость и чат. Кнопка скачивает Markdown-отчёт для аналитика
  с перечнем узлов и запросами недостающих сведений. По умолчанию схема
  показывает топ-150 узлов, а не все 2 248 сразу.
...
Секретный ключ не добавлять в Git. Графовый экран загружает
Tailwind и vis-network через CDN: без интернета схема не отрисуется.
...
с признаком `chronology_ok`: для каждого шага ищет перевод с датой не раньше
предыдущей. Суммы переводов между шагами не сопоставляются, поэтому даже
путь с подходящими датами не доказывает движение именно тех же денег.
...
Графовый экран зависит от внешних библиотек через CDN; автономный показ
без сети пока не проверен. Режим «вся сеть» может быть тяжёлым для браузера.

## 11. Масштабирование до миллиона узлов
...
`index.html`) применяют vis-network и Tailwind. Codex использовался для подготовки
