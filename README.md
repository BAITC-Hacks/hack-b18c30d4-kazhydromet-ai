...
добавьте его в «Перечень на проверку» и скачайте Markdown.
...
- **Перечень и отчёт.** В карточке нажмите «В перечень на проверку».
  ...
  отменить удаление и скачать Markdown именно для выбранных gid через
  `GET /api/report?gids=...`. До 100 идентификаторов сохраняются только
  ...
  в PDF через диалог печати браузера.
