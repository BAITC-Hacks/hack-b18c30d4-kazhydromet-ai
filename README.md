...
- Один локальный запуск `app.pipeline` читает три Parquet-файла, строит
  ...
  Сохранённый `out/summary.json`: **2 248 узлов, 3 119 связей, 72 кластера,
  топ-50, 3,8 секунды внутри расчёта**. На другом ноутбуке время нужно
  измерить снова.
...
- Экран `web/graph.html` объединяет схему сети, список приоритетов,
  карточку узла, кластеры, устойчивость и чат. Отдельный отчёт помогает
  передать аналитику перечень узлов и запросить недостающие сведения.

### Экран аналитика

Адрес <http://localhost:8000> перенаправляет на `web/graph.html`
(`GET /` в `app/main.py`). Ссылка `/graph.html#<gid>` открывает
конкретный узел; поиск принимает полный `gid` или его однозначный суффикс.

- **Схема сети.** Стрелки показывают направление перевода. По умолчанию
  виден топ-150; выбор узла показывает его окрестность на два шага
  (не более 260 узлов), выбор кластера — до 400 его узлов. Переключатель
  «Вся сеть» показывает все 2 248 узлов и может нагружать браузер.
...
- **Рабочие блоки.** Топ-лист из 50 узлов фильтруется по роли;
  карточка показывает «Почему», входящие и исходящие связи, даты и,
  если обнаружены пробелы в данных, «Что запросить дальше».
  Таблица кластеров открывает группу на схеме; блок устойчивости показывает
  крупнейшую компоненту после исключения топ-5, топ-10 и топ-20 узлов.
  Чат предлагает готовые вопросы, показывает ответ и следы вызванных
  инструментов; без ключа работает локальный демо-режим.
...
Нужен Python 3.12 или новее. Команды выполняются из корня репозитория.
...
На Windows после установки пакетов:

```powershell
.\run.ps1
```

Адрес: <http://localhost:8000>. Остановить сервер: `Ctrl+C`.
...
Он пересчитывает данные во временную папку и проверяет сохранённые
`out/*.csv`, но **не сравнивает два прогона побайтно**.
...
|---|---|---|
| `out/nodes_roles.csv` — 2 248 строк | `gid`, `role`, `role_score`, `cluster_id`, `priority_score`, `evidence` (до 200 символов) | `depth`, `is_seed`, `truncated`, степени и суммы входа/выхода, числа переводов, `pass_through`, PageRank, HITS, посредничество, метрики исходных клиентов, циклы и временные признаки |
...
`out/graph.json` содержит узлы, направленные рёбра, пороги и расчёт
...
| `I>0, O>0, p<0,2`, не seed | `terminal` | 0,55 | «отдал дальше 16% полученного» |
| `O>0`, seed | `transit` | 0,45 | «seed (входящие вне выгрузки): переслал 1 505 966 ₸ 4 получ.» |
...
1. **Обрыв четвёртого колена.** Из него исходящие не выгружались. У 444 узлов
   ...
   банка, месяца и порога. У трёх `consolidator` четвёртого колена нынешний
   `evidence` содержит долю «отдал 0%»; её не следует трактовать как удержание.
...
4. **Направление.** Степени, роль, пути и PageRank вычисляются на направленном
   графе. Только Louvain для сообществ использует неориентированную проекцию.
   Суммы встречных рёбер в проекции складываются.
...
`/api/graph` возвращает `gid` строкой; `/api/node/{gid}` даёт `evidence`,
соседей и `gaps`. `/api/path` отдаёт до пяти кратчайших направленных путей
...
`/api/top` возвращает `evidence`, `why` и глобальный `rank` даже при
фильтре по роли.
...
за **01.07–31.07.2026**, с порогом **5 000 ₸**. Внешние и более
...
## 12. Масштабирование до миллиона узлов
...
## 13. Технологии и вклад команды

Python, pandas, pyarrow, NetworkX, SciPy, FastAPI и OpenAI Agents SDK
используются в текущем коде. Оба графовых экрана (`graph.html` и
`index.html`) применяют vis-network и Tailwind; основной `graph.html`
берёт библиотеки из `web/vendor/`. Codex использовался для подготовки
документации и проверки результатов. Команда применяла Claude Code
в ходе разработки; конкретный вклад в код следует сверить с авторами.
